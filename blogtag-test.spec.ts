...
import { test,  Page, expect, Browser, webkit, firefox, BrowserContext, } from '@playwright/test';
import { chromium } from 'playwright';
import * as fs from 'fs';

  function getBlogUrls(): string[] {
    const filePath:string = "./tests/source/blog-urls-tag-prod.csv"
    const urls: string[] = [];
...
test.describe('Blog Tag tests', () => {
    ...
    urls.forEach((url, index)=> {
        test(`Test: ${url}`, async ({browserName}) => {
      ...
      //       // Define the new tags to check
              const unifiedTags = [  'Cloud',  'azure',  'AWS',  'Tech Operations',  'AI & Data',  'Business & Leadership',  'Software Development',  'Cybersecurity',  'News',  'Upskilling',  'chrome developer tools',  'kubernetes',  'python',  'power bi',  'react',  'javascript',  'docker',  'ruby',  'sql',  'salesforce',  'unity',  'r',  'java',  'devops',  'entity framework core',  'angluar',  'c++',  'c#','Guides','Pluralsight One'];

      //       // Define the old tags to checl
              const oldTags = [  'Public Sector',  'IT Ops',  'Engineering Leadership',  'Data',  'Security',  'Business',  'Developer Experience',  'Professional Development',  'Software Delivery Process',  'Team Development',  'Product & UX',  'AI & Machine Learning',  'Learning & Development', ];
